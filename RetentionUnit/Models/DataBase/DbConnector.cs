...
using RetentionUnit.Models.DataBase;
    ...
    internal sealed class DbConnector
    {
        ...
        */
        ...
        public int Execute(string commandName, QueryParams parameters, int timeout = 90)
        {
            ...
            {
                using (OracleCommand cmd = new OracleCommand(commandName, conn))
                {
                    ...
                    if (parameters != null)
                    {
                        ...
                        Log.Info(
                            $"Executing DB non-query command “{commandName}” with parameters {parameters.InParams.Aggregate(string.Empty, (acc, x) => acc + $"{x.Name}: {(x.Value ?? string.Empty).ToString().Replace("\n", " ")}; ")}");
                    }
                    else
                    {
                        Log.Debug($"Executing DB command “{commandName}” without parameters");
                    }
                    ...
                    {
                        Log.Error(
                            "Stored procedure " + commandName + " returned an error with code " + ex.ErrorCode, ex);
            ...
            byte timeout = 60)
        {
            ...
            {
                using (var cmd = new OracleCommand(commandName, conn))
                {
                    try
                    {
                        cmd.CommandTimeout = timeout;
                        ...
                        if (parameters != null)
                        {
                            ...
                            Log.Info(
                                $"Executing DB command “{commandName}” with parameters {parameters.InParams.Aggregate(string.Empty, (acc, x) => acc + $"{x.Name}: {(x.Value ?? string.Empty).ToString().Replace("\n", " ")};")}");
                        }
                        else
                        {
                            Log.Info("Executing DB command “{0}” without parameters", commandName);
                        }
                    ...
                    {
                        Log.Error("Database procedure " + commandName + " failed with error " + ex.Message, ex);
        ...
        private static bool IsRaw(string commandName)
        {
            return commandName.StartsWith("select", StringComparison.OrdinalIgnoreCase) ||
                   ...
                   commandName.StartsWith("delete", StringComparison.OrdinalIgnoreCase) ||
